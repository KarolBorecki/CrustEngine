...
Triangle *Renderer::ProjectTriangle(Triangle *tri, Vector3 *pos, Vector3 *rot, Camera *cam, Vector3 *lightDir, bool projectLight)
{
  ...
  // Logger::Log("  ->Normal = %lf,%lf,%lf normalLen = %lf", normal.X(), normal.Y(), normal.Z(), normalLen);
  normal /= normalLen;
  ...
  // Logger::Log("Checking dot product(%lf)...", dotProduct);
  ...
  if (dotProduct < 0.0)
  {
    ...
    Logger::Log("Moved: P1(%lf, %lf, %lf) P2(%lf, %lf, %lf) P3(%lf, %lf, %lf)", movedP1.X(), movedP1.Y(), movedP1.Z(), movedP2.X(), movedP2.Y(), movedP2.Z(), movedP3.X(), movedP3.Y(), movedP3.Z());

    Logger::Log("Extended: P1(%lf, %lf, %lf, %lf) P2(%lf, %lf, %lf, %lf) P3(%lf, %lf, %lf, %lf)", extendedP1.X(), extendedP1.Y(), extendedP1.Z(), extendedP1.W(), extendedP2.X(), extendedP2.Y(), extendedP2.Z(), extendedP2.W(), extendedP3.X(), extendedP3.Y(), extendedP3.Z(), extendedP3.W());

    extendedP1 *= *projMat; // Wynik tego mnozenia jest zly
    extendedP2 *= *projMat;
    extendedP3 *= *projMat;

    Logger::Log("Projected: P1(%lf, %lf, %lf, %lf) P2(%lf, %lf, %lf, %lf) P3(%lf, %lf, %lf, %lf)", extendedP1.X(), extendedP1.Y(), extendedP1.Z(), extendedP1.W(), extendedP2.X(), extendedP2.Y(), extendedP2.Z(), extendedP2.W(), extendedP3.X(), extendedP3.Y(), extendedP3.Z(), extendedP3.W());
    ...
    Logger::Log("Befor normalising: P1(%lf, %lf) P2(%lf, %lf) P3(%lf, %lf)", projectedP1.X(), projectedP1.Y(), projectedP2.X(), projectedP2.Y(), projectedP3.X(), projectedP3.Y());
    ...
    Logger::Log("Drawing P1(%lf, %lf) P2(%lf, %lf) P3(%lf, %lf)", projectedP1.X(), projectedP1.Y(), projectedP2.X(), projectedP2.Y(), projectedP3.X(), projectedP3.Y());
...
void Renderer::RecalculateProjectionMatrix(Camera *cam)
{
  ...
  for (int y = 0; y < projMatrix->Height(); y++)
  {
    for (int x = 0; x < projMatrix->Width(); x++)
    {
      Logger::LogInline("%lf ", (*projMatrix)[x][y]);
    }
    Logger::Log("");
  }
}
