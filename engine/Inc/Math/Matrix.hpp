 ...
 */
    ...
    public:
        ...
        static constexpr uint16_t MAX_MATRIX_SIZE{
                4096}; //!< Maximum size of a matrix that calculation will be performed on. This is a theoreticall value and if some calculation will be done on bigger matrix bad things happens.

    protected:
        uint16_t _height{0};    //!< Amount of Height in the matrix.
        uint16_t _width{0};     //!< Amount of Width in the matrix.
        uint16_t _totalSize{
                0}; //!< Total size is a multiplied values of a height and a width. It is a count f elements in an array of #mat.

        T *p_mat{
                nullptr}; //!< Array of matrix's values. It is linear despite matrix being 2-dimensional. It is more efficiently.
    ...
    Matrix<E> &Matrix<E>::operator+=(const Matrix<E> &other) noexcept {
        if (_height != other.Height() || _width != other.Width()) {
            ...
            ExceptionsHandler::ThrowWarning("Trying to add matrixes with different sizes! (%zu x %zu) | (%zu x %zu)",
                                            _height, _width, other._height, other._width);
            return *this;
        }
    ...
    Matrix<E> &Matrix<E>::operator+=(std::initializer_list<E> l) noexcept {
        if (_totalSize != l.size()) {
            ExceptionsHandler::ThrowWarning("Trying to add matrixes with different sizes! (%d) | {%d}", _totalSize,
                                            l.size());
            return *this;
        }
    ...
    Matrix<E> &Matrix<E>::operator-=(const Matrix<E> &other) noexcept {
        if (_height != other._height || _width != other._width) {
            ExceptionsHandler::ThrowWarning(
                    "Trying to substract matrixes with different sizes! (%zu x %zu) | (%zu x %zu)", _height, _width,
    ...
    Matrix<E> &Matrix<E>::operator-=(std::initializer_list<E> l) noexcept {
        if (_totalSize != l.size()) {
            ExceptionsHandler::ThrowWarning("Trying to substract matrixes with different sizes! (%d) | {%d }",
                                            _totalSize, l.size());
            return *this;
        }
    ...
    Matrix<E> &Matrix<E>::operator*=(const Matrix<E> &other) noexcept {
        if (_width != other._height) {
            ...
            ExceptionsHandler::ThrowWarning(
                    "Trying to multiply matrixes with not compatible sizes! (%zu x %zu) | (%zu x %zu)", _height, _width,
                    other._height, other._width);
            return *this;
        }
    ...
    Matrix<E> &Matrix<E>::operator*=(std::initializer_list<E> l) noexcept {
        ...
        if (_width != otherHeight) {
            ExceptionsHandler::ThrowWarning(
                    "Trying to muyltiply matrixes with not comaptible sizes! (%d x %d) | {%d x %d}", _height, _width,
    ...
    Matrix<E> &Matrix<E>::operator/=(const Matrix<E> &other) noexcept {
        if (_height != other._height || _width != other._width) {
            ExceptionsHandler::ThrowWarning("Trying to divide matrixes with different sizes! (%zu x %zu) | (%zu x %zu)",
                                            _height, _width, other._height, other._width);
            return *this;
        }
